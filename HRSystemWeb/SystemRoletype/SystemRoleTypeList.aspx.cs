...
namespace HRSystemWeb.SystemRoleType
{
    public partial class SystemRoleTypeList : PageBase
    {
        ...
        private void FillGrid(int aPageNumber)
        {
            ...
            SystemRoleTypeBL bl = new SystemRoleTypeBL(0);
            bl.FetchAll(ds);
            ds.Tables[0].Columns.Add("JumpParam");
            foreach (DataRow item in ds.Tables[0].Rows)
            {
                item["JumpParam"] = "SystemRoleTypeEdit.aspx?" + URLMessage.Encrypt("action=update&SystemRoleType=" + item["SystemRoleType"]);
            }
