...
{
    public partial class SaleReportPeriodic : PageBase
    {
        ...
        protected new void Page_Load(object sender, System.EventArgs e)
        {
            ...
            PageName = "Periodic Sale Transaction Report ";
        ...
        private void FillGrid(int aPageNumber)
        {
         ...
         //   ItemTranBL.FetchSoldForperiodicByUser(DSItem, drpBranch.SelectedValue.ToString(), WinUtility.Cast(drpItem.SelectedValue, 0), WinUtility.Cast(txtStartDate.Text, DateTime.Now), WinUtility.Cast(txtEndDate.Text, DateTime.Now), SessionContext.SystemUser);
            ...
            grdRequests.DataSource = DSItem.Tables[0].DefaultView;
            grdRequests.DataBind();

            // Add the footer row with the total amount
            if (grdRequests.FooterRow != null)
            {
                grdRequests.FooterRow.Cells[0].Text = "Total";
                grdRequests.FooterRow.Cells[0].ColumnSpan = 7; // Adjust this based on your columns
                grdRequests.FooterRow.Cells[0].HorizontalAlign = HorizontalAlign.Right;
                grdRequests.FooterRow.Cells.RemoveAt(1); // Remove unnecessary cells based on your columns
                grdRequests.FooterRow.Cells.RemoveAt(1);
                grdRequests.FooterRow.Cells.RemoveAt(1);
                grdRequests.FooterRow.Cells.RemoveAt(1);
                grdRequests.FooterRow.Cells.RemoveAt(1);
                grdRequests.FooterRow.Cells.RemoveAt(1);

                grdRequests.FooterRow.Cells[1].Text = totalAmount.ToString("N2");
                grdRequests.FooterRow.Cells[1].HorizontalAlign = HorizontalAlign.Right;
            }
            ...
            grdRequests.PageIndex = aPageNumber > grdRequests.PageCount ? 0 : aPageNumber;
            SessionContext.GridPage = grdRequests.PageIndex;
            // objRequest = null;
            ds = null;
        }

        protected void grdgrdRequests_PageIndexChanged(object sender, DataGridPageChangedEventArgs e)
        {
        ...
        {
            grdRequests.PageIndex = e.NewPageIndex > grdRequests.PageCount ? 0 : e.NewPageIndex;
            FillGrid(e.NewPageIndex);
        }
        ...
        private void ExportGridViewToCSV(GridView gridView, string fileName)
        {
            ...
            {
                csv.Append(gridView.HeaderRow.Cells[i].Text);
        ...
        protected void ExportToExcel()
    {
        Response.Clear();
        Response.Buffer = true;
        Response.AddHeader("content-disposition", "attachment;filename=PeriodicSaleTransactionReport.xls");
        Response.Charset = "";
        ...
        using (StringWriter sw = new StringWriter())
        {
            ...
            // To Export all pages
            grdRequests.AllowPaging = false;
            FillGrid(grdRequests.PageIndex);

            grdRequests.HeaderRow.BackColor = System.Drawing.Color.White;
            foreach (TableCell cell in grdRequests.HeaderRow.Cells)
            {
                cell.BackColor = grdRequests.HeaderStyle.BackColor;
            }
            foreach (GridViewRow row in grdRequests.Rows)
            {
                ...
                foreach (TableCell cell in row.Cells)
                {
                    if (row.RowIndex % 2 == 0)
                    {
                        cell.BackColor = grdRequests.AlternatingRowStyle.BackColor;
                    }
                    else
                    {
                        cell.BackColor = grdRequests.RowStyle.BackColor;
                    }
                    cell.CssClass = "textmode";
                }
            }

            grdRequests.RenderControl(hw);
