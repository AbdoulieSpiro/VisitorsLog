...
/* -------------------------------------------------------------------------- */
  ...
  var totalSalesInit = function totalSalesInit(chartData) {
    ...
    function getFormatter(params) {
      ...
      return "".concat(months[date.getMonth()], " ").concat(date.getDate(), ", ").concat(value);
    }

    if ($echartsLineTotalSales) {
      // Process chartData to format required by ECharts
      var xAxisData = chartData.map(item => item.Year);
      var seriesData = chartData.map(item => item.Amount);
      ...
      // Change chart options according to the selected month
      var monthSelect = document.querySelector(SELECT_MONTH);
      if (monthSelect) {
        monthSelect.addEventListener('change', function (e) {
          var month = e.currentTarget.value;
          var data = chartData.filter(item => months.indexOf(item.Month) === parseInt(month));
          chart.setOption({
            tooltip: {
              formatter: function formatter(params) {
...
var totalSalesInits = function totalSalesInits() {
    var ECHART_LINE_TOTAL_SALES = '.echart-line-total-sale';
    var SELECT_MONTH = '.select-month';
    var $echartsLineTotalSales = document.querySelector(ECHART_LINE_TOTAL_SALES);
    var months = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'];
    function getFormatter(params) {
