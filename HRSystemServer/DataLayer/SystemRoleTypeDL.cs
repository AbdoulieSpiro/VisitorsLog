...
namespace HRSystemServer.DataLayer
{
    ...
    public class SystemRoleTypeDL : DataLayerBase, IDataLayer
    {
        private static SystemRoleTypeDL _instance = null;


        //****************************************************
        //****************************************************
        private SystemRoleTypeDL()
        {
            _sqlEntity = SqlEntity.tbSystemRoleType;
        }


        //****************************************************
        //****************************************************
        public static SystemRoleTypeDL GetInstance()
        {
            if (_instance == null)
            {
                _instance = new SystemRoleTypeDL();
            }
            return _instance;
        }
        ...
        public new void Execute(DataLayerMessage aMessage)
        {
            switch (aMessage.SqlAction)
            {
                ...
                case SqlAction.FetchForRole:
                    FetchForRole(aMessage);
                    break;
                case SqlAction.FetchForSystemRoleType:
        ...
        protected override SqlCommand GetCommand(SqlConnection aSqlConnection, SqlAction aSqlAction)
        {
            ...
            switch (aSqlAction)
            {
                case SqlAction.Insert:
                    ...
                    result.Parameters.Add("@SystemRoleTypeX", SqlDbType.VarChar);
                    result.Parameters.Add("@SecurityProfile", SqlDbType.Int);
                    result.Parameters.Add("@AllowAllSiteAccess", SqlDbType.Bit);
                    result.Parameters.Add("@AllowAllCompanyAccess", SqlDbType.Bit);
        ...
        //****************************************************
        //****************************************************
        public void FetchForRole(DataLayerMessage aMessage)
        {
            SqlDataAdapter da = GetDataAdapter(aMessage.SqlConnection);
            da.SelectCommand = GetCommand(aMessage.SqlConnection, aMessage.SqlAction);
            aMessage.ApplyParams(da.SelectCommand.Parameters);
